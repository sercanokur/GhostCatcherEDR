...
// Scan is the top-level persistence entry point. It fans out to each
// sub-detector (authorized_keys, cron, systemd, shellrc, pam, sudoers, sshd,
// users, kernel-modules, ld.so.conf) and concatenates their events.
//
// Any sub-detector returning a non-nil error aborts the batch; best-effort
// detectors return (nil, nil) on non-existent paths so the caller never sees
// transient errors on non-Linux hosts.
func Scan(cfg *config.Config, snap *baseline.Snapshot, pack *rules.Pack, agentVer string) ([]event.Event, error) {
	...
	sd, err := scanSystemdUnits(cfg, snap, pack, agentVer, now)
	if err == nil {
		out = append(out, sd...)
	}
	out = append(out, scanShellRC(cfg, snap, pack, agentVer, now)...)
	out = append(out, scanPAM(cfg, snap, pack, agentVer, now)...)
	out = append(out, scanSudoers(cfg, snap, pack, agentVer, now)...)
	out = append(out, scanSSHD(cfg, snap, pack, agentVer, now)...)
	out = append(out, scanUsers(cfg, snap, pack, agentVer, now)...)
	out = append(out, scanKernelModules(cfg, snap, pack, agentVer, now)...)
	out = append(out, scanLDConf(cfg, snap, pack, agentVer, now)...)
...
func passwdUsers() ([]passwdEntry, error) {
	...
	if err != nil {
		...
		if u, e := user.Current(); e == nil {
			fallback = append(fallback, passwdEntry{name: u.Username, dir: u.HomeDir, uid: u.Uid})
		}
		return fallback, nil
	}
	var res []passwdEntry
	for _, line := range strings.Split(string(out), "\n") {
		...
		res = append(res, passwdEntry{name: p[0], uid: p[2], dir: p[5], shell: p[6]})
	}
	return res, nil
}
...
// BuildBaselineCron captures all current cron lines across every canonical
// cron surface (including anacron and cron.{hourly,daily,weekly,monthly}) so
// post-commit deltas can flag only genuinely new entries.
